...
func (n *Node) Bootstrap2() {
	...
	for _, bootstrapNode := range bootstrapNodes {
		...
		byte32 := message.StringToByte32(n.GetID())
		bootstrapMessage := message.NewDHTGetMessage(byte32)
		...
		desSuccessMsg, _ := deserializedResponse.(*message.DHTSuccessMessage)
		des, _ := dht.Deserialize(desSuccessMsg.Value)
		for _, node := range des.Nodes {
			n.AddPeer(node.ID, node.IP, node.Port)
		}
	}
}
...
func (n *Node) tryBootstrap2() bool {
	bootstrapNodes := n.Config.BootstrapNodes
	...
	for _, bootstrapNode := range bootstrapNodes {
		...
		case *message.DHTBootstrapReplyMessage:
			...
			//TODO: HERE ADD THE BOOTSTRAP NODE ADDING LOGIC
			for _, peer := range response.ParseNodes() {
				peerid := GenerateNodeID(peer.IP, peer.Port)
				n.AddPeer(peerid, peer.IP, peer.Port)
			}
		case *message.DHTSuccessMessage:

			desSuccessMsg, _ := dht.Deserialize(response.Value)
			for _, node := range desSuccessMsg.Nodes {
				n.AddPeer(node.ID, node.IP, node.Port)
			}
		}
